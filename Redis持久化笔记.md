...
本文来自：https://pdai.tech/md/db/nosql-redis/db-redis-x-rdb-aof.html
...
从严格意义上说，Redis服务提供四种持久化存储方案：`RDB`、`AOF`、`虚拟内存（VM）`和　`DISKSTORE`。`虚拟内存（VM）`方式，从Redis Version 2.4开始就被官方明确表示不再建议使用。

至于`DISKSTORE`方式，是从Redis Version 2.8版本开始提出的一个存储设想，到目前为止Redis官方也没有在任何stable版本中明确建议使用这用方式。在Version 3.2版本中同样找不到对于这种存储方式的明确支持。

最关键的是目前官方文档上能够看到的Redis对持久化存储的支持明确的就只有两种方案（https://redis.io/topics/persistence）：`RDB`和`AOF`。

# RDB持久化

> RDB就是Redis DataBase的缩写，中文名为快照（内存快照），RDB持久化是把当前进程数据生成快照保存到磁盘上的过程，由于是某一时刻的快照，那么**快照中的值要早于或者等于内存中的值**。
...
RDB中的核心思路是Copy-on-Write，来保证在进行快照操作的这段时间，需要压缩写入磁盘上的数据在内存中不会发生变化。在正常的快照操作中，一方面Redis主进程会fork一个新的快照进程专门来做这个事情，这样保证了Redis服务不会停止对客户端包括写请求在内的任何响应。另一方面**这段时间发生的数据变化会以副本的方式存放在另一个新的内存区域，待快照操作结束后才会同步到原来的内存区域。**
...
![img](Redis持久化笔记.assets/redis-x-aof-42.jpg)
...
![img](Redis持久化笔记.assets/redis-x-rdb-2.jpg)
...
![img](Redis持久化笔记.assets/redis-x-aof-4.jpg)
...
![img](Redis持久化笔记.assets/redis-x-aof-1.jpg)

- **AOF重写会阻塞吗**？

AOF重写过程是由后台进程bgrewriteaof来完成的。主线程fork出后台的`bgrewriteaof`子进程，fork会把主线程的内存拷贝一份给bgrewriteaof子进程，这里面就包含了数据库的最新数据。然后，bgrewriteaof子进程就可以在不影响主线程的情况下，逐一把拷贝的数据写成操作，记入重写日志。
...
重写过程总结为：“一个拷贝，两处日志”。在fork出子进程时的拷贝，以及在重写时，如果有新数据写入，主线程就会将命令记录到两个(新的和旧的AOF日志)aof日志内存缓冲区中。如果AOF写回策略配置的是always，则直接将命令写回旧的日志文件，并且保存一份命令至AOF重写缓冲区，这些操作对新的日志文件是不存在影响的。（旧的日志文件：主线程使用的日志文件，新的日志文件：bgrewriteaof进程使用的日志文件）
...
> Redis 4.0 中提出了一个**混合使用 AOF 日志和RDB内存快照**的方法。简单来说，内存快照以一定的频率执行，在两次快照之间，使用 AOF 日志记录这期间的所有命令操作。
...
<img src="Redis持久化笔记.assets/redis-x-aof-5.png" alt="img" style="zoom:50%;" />
...
在 Redis 中，持久化数据的恢复通常依赖重启服务加载 RDB 或 AOF 文件，但某些场景下需要 **不重启 Redis 直接恢复数据**。以下是几种无需重启的恢复方法及其适用场景：

**1. 通过主从复制间接恢复**

**适用场景**：主节点数据损坏，但有从节点或备份文件。

**步骤**：

1. **准备备份数据**：将备份的 RDB/AOF 文件放入从节点的持久化目录。
2. **启动从节点**：确保从节点配置为加载持久化文件（如 `appendonly yes` 或 `save` 规则）。
3. **同步数据到主节点**：
   - 将从节点设置为新的主节点（`SLAVEOF NO ONE`）。
   - 让原主节点重新同步新主节点的数据（`SLAVEOF <new-master-ip> <port>`）。
4. **切换流量**：客户端连接切换到新主节点。
...
**适用场景**：需要将备份数据追加到运行中的 Redis，且允许覆盖或追加部分数据。

**工具与步骤**：

1. **解析 RDB/AOF 文件**：

   - 使用工具如 `redis-rdb-tools` 解析 RDB 文件，生成 Redis 命令：

     ```
     rdb --command protocol dump.rdb > commands.txt
     ```

   - 或直接使用 AOF 文件（需清理无关命令）。

2. **批量导入数据**：

   ```
   cat commands.txt | redis-cli -h <host> -p <port> --pipe
   ```

3. **验证数据一致性**：检查关键数据是否完整。
...
**适用场景**：AOF 文件包含完整操作日志，需重建内存数据。

**步骤**：

1. **确保 AOF 启用**：配置 `appendonly yes`（如果未启用需动态开启）。
...
3. **等待重写完成**：生成新的 AOF 文件（如 `appendonly.aof.1.incr`）。

4. **替换并加载**（可选）：

   - 将备份的 AOF 文件替换为当前文件（需停止写入）。
   - 重启 Redis（此步骤需重启，但可结合主从架构避免）。

**优点**：依赖 Redis 自身机制，风险较低。

**缺点**：若需替换 AOF 文件仍需重启，仅适用于增量修复。

**4. 动态加载 Lua 脚本或模块**

**适用场景**：自定义数据恢复逻辑（需开发支持）。

**步骤**：

1. **开发恢复脚本**：编写 Lua 脚本或 Redis 模块，支持从外部存储（如磁盘、网络）读取数据并插入。

   ```lua
   -- 示例：从文件读取数据并插入
   local data = io.open("/data/backup.txt", "r")
   for line in data:lines() do
       redis.call("SET", line.key, line.value)
   end
   ```
...
2. **执行脚本**：
   ...
   ```
   redis-cli --eval restore.lua
   ```

**优点**：高度定制化。

**缺点**：开发成本高，需谨慎处理并发和性能问题。

**5. 利用 Redis 的 `DEBUG` 命令（风险高，慎用！）**
...
```
# 触发生成新的 RDB 文件（可能丢失当前数据）
redis-cli DEBUG RELOAD
```

**警告**：`DEBUG` 命令可能导致数据丢失或服务不稳定，仅限测试环境使用。
